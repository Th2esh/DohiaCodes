...
if __name__=='__main__':
    ...
    result = myinterface()[0:-1]
    res0 = result + "]"
    res1 = eval(res0)
    print(res1)
    print(res1[0]['relating_order_sn'])
    while (3 > 2):
        if str(temp_code) == str(999):
            ...
            sn_list = []
            for i in range(res1):
                if (res1[i]['goods_number']!= 1):
                    for s in range(int(res1[i]['goods_number'])):
                        check_list.append(res1[i]['barcode'])
                        id_list.append(res1[i]['return_order_id'])
                        sn_list.append(res1[i]['return_order_sn'])
                else:
                    check_list.append(res1[0]['barcode'])
                    id_list.append(res1[0]['return_order_id'])
                    sn_list.append(res1[0]['return_order_sn'])
