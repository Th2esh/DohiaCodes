...
if __name__ == "__main__":
    ...
    if number == str(1):
        ...
        sql = "select goods_barcode.barcode,goods.goods_sn,goods.goods_name,spkcb.sl - spkcb.sl2 as 实时库存 ,spkcb.ck_id_name as 仓库 from spkcb,goods,goods_barcode  where spkcb.goods_sn = goods.goods_sn and spkcb.goods_sn = goods_barcode.sku and spkcb.ck_id in (7,41) "
    elif number == str(4):
        sql = "SELECT goods_barcode.barcode,goods.goods_sn,goods.goods_name,sum(spkcb.sl-spkcb.sl2) AS 实时库存 FROM spkcb,goods,goods_barcode WHERE spkcb.goods_sn=goods.goods_sn AND spkcb.goods_sn=goods_barcode.sku AND spkcb.ck_id IN (23)  GROUP BY goods.goods_sn"
    else:
